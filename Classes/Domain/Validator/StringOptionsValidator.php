...
namespace Digicademy\CHFMap\Domain\Validator;
    ...
    public function isValid(mixed $value): void
    {
        ...
        foreach($this->options['allowed'] as $allowed) {
            if (!is_string($allowed)) {
                throw new InvalidValidationOptionsException($this->translateErrorMessage('validator.stringOptions.allowedStringInvalid', 'chf_map'), 1691677298);
            }
        }

        // Check if value is a string
        if (!is_string($value)) {
            $this->addError($this->translateErrorMessage('validator.stringOptions.invalidString', 'chf_map'), 1691677428);

        // Check if value is one of the allowed values
        } elseif (!in_array($value, $this->options['allowed'])) {
            $this->addError($this->translateErrorMessage('validator.stringOptions.notAllowedValue', 'chf_map'), 1691677607);
        }
    }
}
